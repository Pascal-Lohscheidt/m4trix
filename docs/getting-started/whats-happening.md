...
5. **Client Channel** — Emitted events are published to channels the agent is bound to. The client channel has an `sse()` proxy, so events are streamed as SSE to the browser.
